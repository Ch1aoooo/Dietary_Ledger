        ...
        <Card className="p-5">
          ...
          <div className="mt-4 rounded-xl border border-border/60 bg-muted/30 p-4">
            ...
            <p className="mt-2 text-[13px] leading-relaxed text-muted-foreground">
              我們保留原本的推估——{fmtNum(item.modelEstimate)} {item.typicalUnit}
              （confidence {Math.round(item.confidence)}%），不會覆寫成一個假的確認值。
            </p>
          </div>
        </Card>
      );
      ...
      <Card className="p-5">
        ...
        <div className="mt-4 rounded-xl border border-confirmed/25 bg-confirmed/5 p-4">
          ...
          <div className="mt-3 grid grid-cols-3 gap-3">
            <div>
              <p className="text-2xs text-muted-foreground">Before (estimated)</p>
            ...
            <div className="col-span-2">
              <p className="text-2xs text-muted-foreground">After (confirmed)</p>
              <p className="mt-1 text-lg font-semibold text-confirmed">
                本人食用 {fmtNum(c)} {item.typicalUnit}
              </p>
              <p className="mt-1 text-2xs text-muted-foreground">Source: User Confirmed</p>
            </div>
          </div>
        </div>
      </Card>
  ...
  return (
    <Card className="p-5">
      ...
      <div className="mt-4 flex items-center gap-6 rounded-xl bg-muted/50 px-4 py-3">
        <div>
          <p className="text-2xs text-muted-foreground">Purchased</p>
          <p className="text-2xl font-semibold text-foreground">{fmtNum(item.purchasedQty)}</p>
        </div>
        <div className="h-9 w-px bg-border" />
        <div>
          <p className="text-2xs text-muted-foreground">Model estimate</p>
      ...
      <div className="mt-4">
        <p className="mb-2 flex items-center gap-1.5 text-2xs font-medium uppercase tracking-wide text-muted-foreground">
          <HelpCircle className="h-3.5 w-3.5" /> Reasoning
        </p>
        ...
        {picked != null && (
          <span className="flex items-center gap-1 text-xs text-muted-foreground">
            ...
            {picked === "unsure"
              ...
              : `選取後此紀錄將視為「本人食用 ${picked} ${item.typicalUnit}」`}
          </span>
        )}
      </div>
    </Card>
