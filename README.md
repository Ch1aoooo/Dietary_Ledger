...
英文模式下，送往後端的分析請求會帶 `lang` 參數，讓 AI 產生的「推論理由（reasoning）」與「AI 飲食顧問」的回覆也以英文輸出——這部分無法用靜態對照表處理，改由 `backend/main.py` 依 `lang` 切換系統提示。
...
| 醫療應用 | 團隊目前無醫事專業成員，無法建立合法並值得信賴的背景 | 與醫療及營養專業團隊合作，評估健康管理與預防醫學應用 |
...
| 來源／服務 | 用途 | 使用與授權說明 |
| --- | --- | --- |
| 財政部電子發票整合服務平台 | 發票資料與匯出格式來源 | 使用者自行取得並上傳其有權使用的資料 |
| Google Gemini API | 資料分析測試 | 免費授權 |
| GPT 5.6（Sponsor 技術） | 預計用於資料分析測試 | 未有實際採用情況 |
